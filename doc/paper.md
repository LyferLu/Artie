...
系统的核心用例可概括为：文生图生成、AI擦除、AI扩图、AI重绘、智能选区、去背景、超分辨率、修复人脸、工作区保存与工作区恢复。其中前八项为主要业务功能，工作区保存与恢复贯穿整个处理过程。
...
| 用例名称 | 文生图 |
| 参与者 | 登录用户 |
| 前置条件 | 用户已登录系统，服务端存在可用文生图模型 |
| 基本流程 | 1. 用户进入文生图页面；2. 输入提示词与参数；3. 提交生成请求；4. 系统执行模型推理并反馈进度；5. 系统返回生成结果；6. 用户选择保存、下载或继续发送到其他功能 |
| 后置结果 | 生成图片被加载到当前工作区，可继续处理或保存 |
...
| 参与者 | 登录用户 |
| 前置条件 | 工作区中存在当前图片 |
| 基本流程 | 1. 用户进入 AI擦除页面；2. 在画布上绘制蒙版；3. 选择或保持默认参数；4. 提交擦除请求；5. 系统返回修复结果 |
| 后置结果 | 当前工作图像更新为擦除后的结果，可继续编辑或保存 |
...
| 用例名称 | AI扩图 |
| 参与者 | 登录用户 |
| 前置条件 | 工作区中存在当前图片，且存在支持扩图的模型 |
| 基本流程 | 1. 用户进入 AI扩图页面；2. 设置扩展方向与范围；3. 提交扩图请求；4. 系统完成扩展和补全；5. 返回结果 |
| 后置结果 | 工作区图像更新，图像尺寸与边界内容同步变化 |
...
| 参与者 | 登录用户 |
| 前置条件 | 工作区中存在当前图片 |
| 基本流程 | 1. 用户进入 AI重绘页面；2. 绘制局部区域；3. 输入重绘提示词；4. 提交请求；5. 系统返回局部重绘结果 |
| 后置结果 | 当前工作图像被替换为重绘后的结果 |
...
| 用例名称 | 智能选区 |
| 参与者 | 登录用户 |
| 前置条件 | 工作区中存在当前图片，服务端已启用交互式分割插件 |
| 基本流程 | 1. 用户进入智能选区页面；2. 点击前景点和背景点；3. 系统生成临时选区；4. 用户确认选区；5. 选区转为蒙版供后续处理 |
| 后置结果 | 工作区中形成可用于擦除或重绘的蒙版结果 |
...
| 用例名称 | 去背景 |
| 参与者 | 登录用户 |
| 前置条件 | 工作区中存在当前图片，服务端启用 RemoveBG 插件 |
| 基本流程 | 1. 用户进入去背景页面；2. 选择模型；3. 提交处理请求；4. 系统返回透明背景结果 |
| 后置结果 | 去背景结果可继续发送至其他模块或保存到工作区 |
...
| 用例名称 | 超分辨率 |
| 参与者 | 登录用户 |
| 前置条件 | 工作区中存在当前图片，服务端启用 Real-ESRGAN 插件 |
| 基本流程 | 1. 用户进入超分辨率页面；2. 选择增强模型；3. 提交增强请求；4. 系统执行放大与细节增强；5. 返回结果 |
| 后置结果 | 图像分辨率提升，结果被写入当前工作区状态 |
...
| 用例名称 | 修复人脸 |
| 参与者 | 登录用户 |
| 前置条件 | 工作区中存在当前图片，服务端启用 GFPGAN 或 RestoreFormer 插件 |
| 基本流程 | 1. 用户进入修复人脸页面；2. 选择修复插件；3. 提交修复请求；4. 系统返回人脸增强结果 |
| 后置结果 | 当前图像中的人脸细节得到修复，并可继续保存或衔接到编辑功能 |
