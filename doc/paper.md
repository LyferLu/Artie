...
需要说明的是，对于“用户名重复注册”“提示词为空无法生成”“未上传图片不执行处理”等负例场景，只要系统能够按照预期阻止提交、不给出错误结果或显示相应提示，仍应判定为测试成功；只有当系统实际表现与预期不一致时，才记为测试失败。
...
| TC015 | AI扩图 | 未上传图片 | 1. 直接进入 AI扩图页。<br>2. 不上传图片。 | 无 | 页面显示需先上传图片的空状态提示，系统不执行扩图请求。 |
...
| TC030 | 智能选区 | 未上传图片 | 1. 直接进入“智能选区”页。<br>2. 不上传图片。 | 无 | 页面显示需先上传图片的空状态提示，无法执行交互分割。 |
...
如表 6.12 所示，对系统在弱网络环境和界面一致性方面的补充问题进行测试。

表 6.12 补充问题测试用例

| 用例编号 | 测试项 | 测试标题 | 用例操作 | 用例数据 | 预期结果 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| TC037 | 去背景 | 首次初始化模型时网络受限 | 1. 在本地未缓存去背景模型的环境下启动系统。<br>2. 保持当前环境无法稳定访问 Hugging Face。<br>3. 进入“去背景”页并上传图片。<br>4. 点击“AI 去背景”。 | 模型：briaai/RMBG-1.4<br>网络环境：受限或不稳定<br>图片：product.png | 系统能够正常初始化去背景模型并输出透明背景结果图。 |
| TC038 | 界面文案 | 空状态提示中文化一致性 | 1. 在未上传图片的情况下分别进入 AI扩图、AI重绘和智能选区页面。<br>2. 观察页面空状态提示文案。 | 无 | 系统统一使用中文提示用户先上传图片，界面文案保持一致。 |

## 6.3 测试结果分析

在上述测试环境下，本文对系统核心功能进行了逐项验证。测试结果表明，系统在注册登录、文生图、AI擦除、AI扩图、AI重绘、超分辨率、修复人脸、智能选区和工作区管理等主要业务链路上运行稳定；但在去背景模型首次联网初始化和部分界面提示文案统一性方面，仍存在待优化问题。

如表 6.13 所示，对系统测试结果进行汇总。
...
| TC020 | 去背景结果显示在结果区域 | 去背景结果正确显示在结果区域 | 测试成功 |
...
| TC026 | 输出人脸修复结果 | 系统输出人脸修复结果 | 测试成功 |
...
| TC037 | 系统能够正常初始化去背景模型并输出结果图 | 在模型未缓存且网络受限时，RemoveBG 插件初始化失败，无法返回去背景结果 | 测试失败 |
| TC038 | 系统统一使用中文空状态提示 | AI扩图、AI重绘和智能选区页面仍显示英文提示 `Please upload an image first` | 测试失败 |

综合来看，本系统在用户登录鉴权、八大功能执行、结果展示、工作区保存恢复以及跨模块状态衔接等方面均达到了预期目标，绝大多数用例与预期一致。特别是在文生图结果一次性衔接、插件结果撤回重做、未保存进度确认和工作区恢复等细节交互上，系统表现出了较好的完整性和可用性。但从补充测试结果也可以看出，系统仍存在两类需要继续优化的问题：其一，去背景模型在未预下载且网络受限的环境下，对外部模型源仍有依赖；其二，部分页面空状态提示尚未完成中文化统一。与此同时，高分辨率图像处理和多模型切换对本地硬件资源仍有较高要求，后续还需在模型本地化准备、文案一致性和异常恢复体验方面继续完善。
