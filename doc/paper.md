...
本系统功能模块详细设计围绕创作生成、编辑处理、插件增强、工作区管理和登录支撑五类业务展开。由于不同模块的业务链路和交互方式存在明显差异，下面按照具体功能逐项说明，并使用严格采用 BCE 模式的时序图描述用户、界面、控制器和业务实体之间的协同过程。

### 4.3.1 文生图创作模块设计

文生图创作模块涉及的功能包括参数录入、生成执行、结果展示和后续衔接。由于该模块的核心业务集中在文生图生成环节，本部分在文生图创作模块设计中选择文生图生成功能进行详细描述。

1. 文生图生成：文生图生成是用户从零开始创作图像的核心入口。首先，用户访问文生图界面，并在文生图界面中输入文生图信息，文生图信息包括提示词、反向提示词、图像尺寸、采样步数、采样器和随机种子等。然后，点击生成按钮提交文生图信息。经系统验证文生图信息后，根据验证结果判断是否继续接下来的操作，若验证结果为成功，则调用支持文生图的模型执行推理。经系统生成文生图结果后，根据处理结果反馈不同的文生图结果界面，这些文生图结果包括生成成功和生成失败。文生图生成序列图如图 4.2 所示。
...
control "文生图控制" as C1
entity "文生图信息" as E1
boundary "文生图成功界面" as BS1
boundary "文生图失败界面" as BF1
...
图 4.2 文生图生成序列图（PlantUML 草稿）

### 4.3.2 AI擦除模块设计

AI擦除模块涉及的功能包括加载当前工作图像、绘制蒙版、提交擦除请求和更新处理结果。由于该模块的核心业务集中在擦除处理环节，本部分在 AI擦除模块设计中选择 AI擦除功能进行详细描述。

1. AI擦除：AI擦除用于移除图像中不需要的局部内容并完成智能补全。首先，用户访问 AI擦除界面，系统在 AI擦除界面中加载当前工作图像。然后，用户在画布中绘制待处理区域的蒙版，并点击执行按钮提交 AI擦除信息。经系统验证 AI擦除信息后，根据验证结果判断是否继续接下来的操作，若验证结果为成功，则执行局部修复推理。经系统处理后，根据处理结果反馈不同的 AI擦除结果界面，这些 AI擦除结果包括擦除成功和擦除失败。AI擦除序列图如图 4.3 所示。
...
entity "AI擦除信息" as E2
boundary "AI擦除成功界面" as BS2
boundary "AI擦除失败界面" as BF2
...
图 4.3 AI擦除序列图（PlantUML 草稿）

### 4.3.3 AI扩图模块设计

AI扩图模块涉及的功能包括加载原始图像、设置扩展方向与范围、提交扩图请求以及更新扩图结果。由于该模块的核心业务集中在扩图处理环节，本部分在 AI扩图模块设计中选择 AI扩图功能进行详细描述。

1. AI扩图：AI扩图用于在保持原始图像主体内容相对稳定的前提下，对图像边界进行扩展和补全。首先，用户访问 AI扩图界面，系统在 AI扩图界面中加载当前工作图像。然后，用户在界面中设置扩展方向、扩展范围和相关提示词信息，并点击执行按钮提交 AI扩图信息。经系统验证 AI扩图信息后，根据验证结果判断是否继续接下来的操作，若验证结果为成功，则执行扩图推理。经系统生成扩图结果后，根据处理结果反馈不同的 AI扩图结果界面，这些 AI扩图结果包括扩图成功和扩图失败。AI扩图序列图如图 4.4 所示。
...
entity "AI扩图信息" as E3
boundary "AI扩图成功界面" as BS3
boundary "AI扩图失败界面" as BF3
...
图 4.4 AI扩图序列图（PlantUML 草稿）

### 4.3.4 AI重绘模块设计

AI重绘模块涉及的功能包括局部区域选定、提示词录入、重绘请求提交和结果替换。由于该模块的核心业务集中在局部重绘处理环节，本部分在 AI重绘模块设计中选择 AI重绘功能进行详细描述。

1. AI重绘：AI重绘用于在局部选区范围内按照新的语义提示重新生成内容。首先，用户访问 AI重绘界面，系统在 AI重绘界面中加载当前工作图像。然后，用户绘制局部蒙版区域，并输入新的重绘提示词和其他参数信息，点击执行按钮提交 AI重绘信息。经系统验证 AI重绘信息后，根据验证结果判断是否继续接下来的操作，若验证结果为成功，则执行局部重绘推理。经系统生成局部重绘结果后，根据处理结果反馈不同的 AI重绘结果界面，这些 AI重绘结果包括重绘成功和重绘失败。AI重绘序列图如图 4.5 所示。
...
entity "AI重绘信息" as E4
boundary "AI重绘成功界面" as BS4
boundary "AI重绘失败界面" as BF4

用户 -> B4 : 1: 进入AI重绘页面()
B4 -> B4 : 2: 加载当前工作图像()
用户 -> B4 : 3: 提交AI重绘信息()
B4 -> C4 : 4: 验证AI重绘信息()
C4 -> E4 : 5: 校验局部蒙版与提示词信息()
E4 --> C4 : 6: 返回校验结果()
C4 -> E4 : 7: 生成AI重绘结果()
E4 --> C4 : 8: 返回处理结果()
alt [成功]
...
图 4.5 AI重绘序列图（PlantUML 草稿）

### 4.3.5 去背景模块设计

去背景模块涉及的功能包括当前图像加载、去背景模型选择、插件处理和结果展示。由于该模块的核心业务集中在去背景处理环节，本部分在去背景模块设计中选择去背景功能进行详细描述。

1. 去背景：去背景用于提取图像主体并输出透明背景结果。首先，用户访问去背景界面，系统在去背景界面中加载当前工作图像。然后，用户选择去背景模型并点击执行按钮提交去背景信息。经系统验证去背景信息后，根据验证结果判断是否继续接下来的操作，若验证结果为成功，则调用去背景插件执行主体分离。经系统生成去背景结果后，根据处理结果反馈不同的去背景结果界面，这些去背景结果包括处理成功和处理失败。去背景序列图如图 4.6 所示。
...
control "去背景控制" as C5
entity "去背景信息" as E5
boundary "去背景成功界面" as BS5
boundary "去背景失败界面" as BF5
...
图 4.6 去背景序列图（PlantUML 草稿）

### 4.3.6 超分辨率模块设计

超分辨率模块涉及的功能包括图像加载、超分模型选择、增强请求提交和增强结果展示。由于该模块的核心业务集中在超分辨率处理环节，本部分在超分辨率模块设计中选择超分辨率功能进行详细描述。

1. 超分辨率：超分辨率用于提升图像清晰度并放大输出尺寸。首先，用户访问超分辨率界面，系统在超分辨率界面中加载当前工作图像。然后，用户选择超分模型并设置放大倍数，点击执行按钮提交超分辨率信息。经系统验证超分辨率信息后，根据验证结果判断是否继续接下来的操作，若验证结果为成功，则调用超分辨率插件执行图像增强。经系统生成超分辨率结果后，根据处理结果反馈不同的超分辨率结果界面，这些超分辨率结果包括增强成功和增强失败。超分辨率序列图如图 4.7 所示。
...
entity "超分辨率信息" as E6
boundary "超分辨率成功界面" as BS6
boundary "超分辨率失败界面" as BF6
...
图 4.7 超分辨率序列图（PlantUML 草稿）

### 4.3.7 修复人脸模块设计

修复人脸模块涉及的功能包括当前图像加载、修复插件选择、修复请求提交和结果回写。由于该模块的核心业务集中在修复人脸处理环节，本部分在修复人脸模块设计中选择修复人脸功能进行详细描述。

1. 修复人脸：修复人脸用于改善图像中人物面部细节失真问题。首先，用户访问修复人脸界面，系统在修复人脸界面中加载当前工作图像。然后，用户选择修复插件并点击执行按钮提交修复人脸信息。经系统验证修复人脸信息后，根据验证结果判断是否继续接下来的操作，若验证结果为成功，则调用对应的人脸修复插件执行增强处理。经系统生成修复人脸结果后，根据处理结果反馈不同的修复人脸结果界面，这些修复人脸结果包括修复成功和修复失败。修复人脸序列图如图 4.8 所示。
...
entity "修复人脸信息" as E7
boundary "修复人脸成功界面" as BS7
boundary "修复人脸失败界面" as BF7
...
图 4.8 修复人脸序列图（PlantUML 草稿）

### 4.3.8 智能选区模块设计

智能选区模块涉及的功能包括当前图像加载、前景点与背景点标注、分割请求提交和选区确认。由于该模块的核心业务集中在选区生成环节，本部分在智能选区模块设计中选择智能选区功能进行详细描述。

1. 智能选区：智能选区用于通过点击交互方式快速生成候选蒙版区域。首先，用户访问智能选区界面，系统在智能选区界面中加载当前工作图像。然后，用户在图像中点击前景点和背景点，并点击确认按钮提交智能选区信息。经系统验证智能选区信息后，根据验证结果判断是否继续接下来的操作，若验证结果为成功，则调用交互式分割插件生成候选选区。经系统返回分割结果后，根据处理结果反馈不同的智能选区结果界面，这些智能选区结果包括选区成功和选区失败。智能选区序列图如图 4.9 所示。
...
entity "智能选区信息" as E8
boundary "智能选区成功界面" as BS8
boundary "智能选区失败界面" as BF8
...
图 4.9 智能选区序列图（PlantUML 草稿）

### 4.3.9 用户工作区模块设计

用户工作区模块涉及的功能包括查看工作区列表、保存工作区、恢复工作区和删除工作区。由于用户工作区模块涉及到的功能较多，此部分在用户工作区模块设计中选择查看工作区列表、保存工作区、恢复工作区和删除工作区功能进行详细描述。

1. 查看工作区列表：查看工作区列表是用户管理个人创作记录的基础功能。首先，用户访问“我的作品”界面。然后，系统向后端提交工作区列表查询请求。经系统验证当前用户身份后，根据验证结果判断是否继续接下来的操作，若验证结果为成功，则查询当前用户对应的工作区记录。经系统获取工作区列表结果后，根据查询结果反馈不同的工作区列表界面，这些查询结果包括查询成功和查询失败。查看工作区列表序列图如图 4.10 所示。
...
control "工作区控制" as C9
entity "工作区信息" as E9
boundary "工作区列表成功界面" as BS9
boundary "工作区列表失败界面" as BF9
...
E9 --> C9 : 7: 返回工作区列表结果()
alt [成功]
  C9 -> BS9 : 8: 反馈工作区列表成功结果()
else [失败]
  C9 -> BF9 : 8: 反馈工作区列表失败结果()
end
@enduml
```

图 4.10 工作区列表查询序列图（PlantUML 草稿）

2. 保存工作区：保存工作区用于持久化保存用户当前的图片、参数和处理状态。首先，用户在任意功能页完成生成或编辑后点击保存按钮。然后，前端整理当前标签页、主图、蒙版、预览图及各功能参数状态，并向后端提交工作区保存信息。经系统验证工作区保存信息后，根据验证结果判断是否继续接下来的操作，若验证结果为成功，则写入或更新工作区会话、快照与资源文件。经系统保存工作区信息后，根据保存结果反馈不同的保存工作区结果界面，这些保存结果包括保存成功和保存失败。保存工作区序列图如图 4.11 所示。

```plantuml
@startuml
hide footbox
actor 用户
boundary "保存工作区界面" as B10
control "工作区保存控制" as C10
entity "工作区信息" as E10
boundary "保存工作区成功界面" as BS10
boundary "保存工作区失败界面" as BF10
...
E10 --> C10 : 6: 返回保存结果()
alt [成功]
  C10 -> BS10 : 7: 反馈保存工作区成功结果()
else [失败]
  C10 -> BF10 : 7: 反馈保存工作区失败结果()
end
@enduml
```

图 4.11 工作区保存序列图（PlantUML 草稿）

3. 恢复工作区：恢复工作区用于把历史创作记录重新加载到当前编辑现场。首先，用户访问“我的作品”界面并选择需要恢复的工作区记录。然后，点击继续编辑按钮提交工作区恢复信息。经系统验证工作区恢复信息后，根据验证结果判断是否继续接下来的操作，若验证结果为成功，则读取对应工作区的快照、资源和功能状态信息。经系统完成工作区恢复后，根据恢复结果反馈不同的工作区恢复结果界面，这些恢复结果包括恢复成功和恢复失败。恢复工作区序列图如图 4.12 所示。

```plantuml
@startuml
hide footbox
actor 用户
boundary "工作区恢复界面" as B11
control "工作区恢复控制" as C11
entity "工作区信息" as E11
boundary "工作区恢复成功界面" as BS11
boundary "工作区恢复失败界面" as BF11

用户 -> B11 : 1: 提交工作区恢复信息()
B11 -> C11 : 2: 验证工作区恢复信息()
C11 -> E11 : 3: 校验工作区会话信息()
E11 --> C11 : 4: 返回验证结果()
C11 -> E11 : 5: 读取工作区快照与资源信息()
E11 --> C11 : 6: 返回恢复结果()
alt [成功]
  C11 -> BS11 : 7: 反馈工作区恢复成功结果()
else [失败]
  C11 -> BF11 : 7: 反馈工作区恢复失败结果()
end
@enduml
```

图 4.12 工作区恢复序列图（PlantUML 草稿）

4. 删除工作区：删除工作区用于清理用户不再需要的工作区记录。首先，用户访问“我的作品”界面并选择需要删除的工作区记录。然后，点击删除按钮提交工作区删除信息。经系统验证工作区删除信息后，根据验证结果判断是否继续接下来的操作，若验证结果为成功，则对对应工作区执行删除处理。经系统完成工作区删除后，根据删除结果反馈不同的工作区删除结果界面，这些删除结果包括删除成功和删除失败。删除工作区序列图如图 4.13 所示。

```plantuml
@startuml
hide footbox
actor 用户
boundary "删除工作区界面" as B12
control "工作区删除控制" as C12
entity "工作区信息" as E12
boundary "删除工作区成功界面" as BS12
boundary "删除工作区失败界面" as BF12
...
E12 --> C12 : 6: 返回删除结果()
alt [成功]
  C12 -> BS12 : 7: 反馈删除工作区成功结果()
else [失败]
  C12 -> BF12 : 7: 反馈删除工作区失败结果()
end
@enduml
```

图 4.13 工作区删除序列图（PlantUML 草稿）

### 4.3.10 登录支撑模块设计

登录支撑模块是通用支撑能力中的关键组成部分。通用支撑模块涉及登录鉴权、进度提示、任务取消和结果下载等功能。由于通用支撑模块涉及到的功能较多，此部分在登录支撑模块设计中选择登录功能进行详细描述。

1. 登录：登录是用户正常访问和操作系统的前提条件。用户只有在成功登录系统后，才能够继续使用文生图、图像编辑和工作区管理等相关业务功能。首先，用户访问登录界面，并在登录界面中输入登录信息，登录信息包括用户名与密码。然后，点击登录按钮提交登录信息。经系统验证登录信息后，根据验证结果判断是否继续接下来的操作，若验证结果为成功，则生成登录会话信息并建立当前用户状态。经系统完成登录处理后，根据处理结果反馈不同的登录结果界面，这些登录结果包括登录成功和登录失败。登录序列图如图 4.14 所示。
...
control "登录控制" as C13
entity "用户信息" as E13
boundary "登录成功界面" as BS13
boundary "登录失败界面" as BF13
...
系统 CDM 如图 4.15 所示，Mermaid 草稿如下。
