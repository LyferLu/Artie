...
| TC015 | AI扩图 | 未上传图片 | 1. 直接进入 AI扩图页。<br>2. 不上传图片。 | 无 | 页面提示先上传图片，系统不执行扩图请求。 |
...
| TC030 | 智能选区 | 未上传图片 | 1. 直接进入“智能选区”页。<br>2. 不上传图片。 | 无 | 页面提示先上传图片，无法执行交互分割。 |
...
在上述测试环境下，本文对系统核心功能进行了逐项验证。测试结果表明，注册登录、文生图、AI擦除、AI扩图、AI重绘、去背景、超分辨率、修复人脸、智能选区和工作区管理等功能均能够按照设计要求完成交互流程，系统在主要业务链路上运行稳定。
...
综合来看，本系统在用户登录鉴权、八大功能执行、结果展示、工作区保存恢复以及跨模块状态衔接等方面均达到了预期目标。特别是在文生图结果一次性衔接、插件结果撤回重做、未保存进度确认和工作区恢复等细节交互上，系统表现出了较好的完整性和可用性。与此同时，测试过程中也能看出本地模型加载和高分辨率图像处理仍受硬件条件影响较大，后续仍需在模型调度效率、资源占用优化和异常恢复体验方面继续完善。
