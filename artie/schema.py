...
class ApiConfig(BaseModel):
    ...
    quality: int
    # LRU model cache settings
    max_cached_models: int = Field(3, description="Maximum number of models to keep in LRU cache")
    max_vram_usage_gb: Optional[float] = Field(None, description="Maximum VRAM usage in GB (default: 85% of GPU total)")
    # Auth & database settings
    disable_auth: bool = Field(False, description="Disable authentication (single-user local mode)")
    auth_secret_key: str = Field("CHANGE_ME_IN_PRODUCTION", description="Secret key for JWT signing")
    db_path: Optional[Path] = Field(None, description="Custom SQLite database file path")
...
class ServerConfigResponse(BaseModel):
    ...
    isDesktop: bool
    samplers: List[str]
    cachedModels: List[str] = Field(default_factory=list)
